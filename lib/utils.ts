...
export function generateSoulmatePrompt(
  ...
  return `Create a single head-and-shoulders portrait drawing of a ${gender} (${ethnicity} ethnicity) in the exact same house style across all generations: a graphite/sepia pencil sketch on warm cream sketchbook paper, subtle paper grain visible. Square format (1:1), centered composition, straight-on view, eyes level with the horizontal center, soft even studio lighting, neutral expression, mouth closed, small catchlights in both eyes. Delicate cross-hatching and fine line work; light construction lines may remain; gentle tonal shading with no harsh contrast. Render with varied line weight like HB–2B pencil on textured paper for skin, hair, and fabric. Keep the crop and scale identical every time: include the subject from collarbone to top of head, shoulders slightly visible, no hands; head size must fill approximately 70–75% of the frame, leaving a consistent margin of about 10% on all sides; simulate a natural 50mm portrait perspective with no distortion, no tilt, and symmetrical alignment. Lighting must be uniform and repeatable: a single soft key light slightly above eye level, frontal, with minimal shadow under the chin and beneath the nose; no dramatic rim lights, no hard shadows, and no directional spot effects; maintain mid-contrast tonality with gentle highlight roll-off. Linework and shading rules are fixed: skin tones built with tight cross-hatching and light smudging for gradients; darker cross-hatch under cheekbones, lower lip, and jawline; smooth shading around the eyes and forehead; hair rendered with layered pencil strokes that follow growth direction—no solid fills, no ink outlines; brows and lashes defined but subtle, irises crisp with a single small highlight per eye. Wardrobe is identical across all outputs: a plain dark turtleneck rendered in pencil with soft mid-tone shading only, no patterns or textures, no logos, no zipper or seams emphasized, and no jewelry, piercings, hats, or glasses; hair may vary naturally with the subject but should not obscure the eyes or exceed the frame. Background is always the same neutral medium: blank cream paper only, with faint paper grain and extremely subtle background cross-hatching if needed for tone; absolutely no props, no text, no captions, no signature, no watermark, no logo, no border, and no color wash—just pencil on paper. Palette is locked: warm sepia graphite lines on cream paper, roughly #6F5B4A line tone on #EFE6D3 paper tone; no additional colors, filters, or tints; do not introduce digital painting, watercolor, or ink—preserve the dry-media pencil look. Consistency requirements that must never change: the same paper texture, the same sepia pencil palette, the same centered straight-on framing, the same head-size and margin ratio, the same plain dark turtleneck, the same even lighting, the same cross-hatching technique, and the same collarbone-to-head composition; produce only one subject per image. The only attributes that may vary are the subject’s natural appearance derived from the inputs: ${gender} and ${ethnicity}; reflect these respectfully through appropriate facial structure, skin tone range, and hair type/texture without caricature or stereotype; keep makeup minimal and natural; facial hair only if naturally consistent with the subject and kept subtle in pencil texture. Negative constraints (hard exclusions): no color, no photoreal photography, no CGI or painterly strokes, no dramatic chiaroscuro, no heavy outlines, no patterns, no accessories, no background elements, no alternative crops, no side angles or profiles, no wide-angle distortion, no borders, no text, no UI artifacts. Output specification: a single square (1:1) portrait at high resolution, ultra-clean with no extraneous elements, from collarbone up, consistent with all rules above.`;
} 
